...
export default function AnnouncementsPage() {
  ...
  const getCategoryBadge = (category) => {
    switch(category?.toLowerCase()) {
      ...
      case 'security': 
        return 'bg-red-100 text-red-700 text-red-400';
      case 'community': 
        return 'bg-green-100 text-green-700 text-green-400';
      default: 
        return 'bg-[#1a1d23] text-[#1241a1] bg-[#1241a1] text-[#1241a1]';
    }
  };
      ...
      <Card>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="text-left bg-[#1a1d23]/60 bg-[#1a1d23]">
            ...
            <tbody className="divide-y-0">
                {isLoading ? (
                  ...
                  announcements.map((ann, idx) => (
                    <tr key={ann._id || ann.id || idx} className="group hover:bg-[#1a1d23] hover:bg-[#2a2d33] transition-all cursor-pointer">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-md bg-[#0d0f13] shrink-0 flex items-center justify-center transition-colors group-hover:bg-[#1241a1] group-hover:text-white">
          ...
          {/* Mobile Card View */}
          <div className="md:hidden divide-y divide-[#2a2d33] divide-[#2a2d33]">
            {isLoading ? (
              ...
              announcements.map((ann, idx) => (
                <div key={ann._id || ann.id || idx} className="group p-4 space-y-3 active:bg-[#1a1d23] active:bg-[#2a2d33] transition-all mb-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div className="size-10 rounded-md bg-[#0d0f13] flex items-center justify-center shrink-0 transition-colors group-hover:bg-[#1241a1] group-hover:text-white">
                        {ann.type?.toLowerCase() === 'maintenance' ? <Wrench className="w-5 h-5" /> : 
                         ann.type?.toLowerCase() === 'security' ? <Shield className="w-5 h-5" /> : 
                         ann.type?.toLowerCase() === 'community' ? <Users className="w-5 h-5" /> : 
                         <Info className="w-5 h-5" />}
