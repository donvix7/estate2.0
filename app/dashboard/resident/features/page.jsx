  ...
  return (
    <div className="max-w-7xl mx-auto pb-12 animate-in fade-in duration-700">
          ...
          {accessTabs.map((tab, index) => (
            <div key={index} className="flex flex-col items-center justify-start w-full">
              <Link href={tab.href} className="flex flex-col items-center gap-2 w-full group">
                <TechCard
                  noPadding
                  className="w-[80px] h-[80px] sm:w-[92px] sm:h-[92px] rounded-2xl cursor-pointer hover:bg-[#2a2d33]/50 transition-colors flex flex-col items-center justify-center"
                  hoverEffect={false}
                >
                  <div className="w-12 h-12 sm:w-14 sm:h-14 bg-blue-900/40 rounded-full flex items-center justify-center">
                    <tab.icon className="w-6 h-6 sm:w-7 sm:h-7 text-blue-400 group-hover:scale-110 transition-transform duration-300" />
                  </div>
                </TechCard>
                <span className="text-xs sm:text-sm font-bold text-[#8a8f98] font-heading text-center leading-tight group-hover:text-white transition-colors">
                  {tab.name}
                </span>
              </Link>
            </div>
      ...
      <div className="hidden md:block text-sm p-4 max-w-7xl mx-auto mt-6">
        <p className="text-xl font-bold mb-6 text-white font-heading">Access</p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-12">
          {accessTabs.map((tab, index) => (
            <Link href={tab.href} key={index} className="group">
              <TechCard className="cursor-pointer hover:bg-[#2a2d33]/50 transition-colors h-full flex flex-col justify-center">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 md:w-12 md:h-12 bg-blue-900/40 rounded-xl flex items-center justify-center shrink-0">
                    <tab.icon className="w-5 h-5 md:w-6 md:h-6 text-blue-400 group-hover:scale-110 transition-transform duration-300" />
                  </div>
                  <span className="text-sm md:text-base font-bold text-white font-heading group-hover:text-white transition-colors">
                    {tab.name}
                  </span>
                </div>
              </TechCard>
        ...
        <p className="text-xl font-bold mb-6 text-white font-heading">Communication</p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-12">
          {communicationTabs.map((tab, index) => (
            <Link href={tab.href} key={index} className="group">
              <TechCard className="cursor-pointer hover:bg-[#2a2d33]/50 transition-colors h-full flex flex-col justify-center">
        ...
        <p className="text-xl font-bold mb-6 text-white font-heading">Payment</p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-12">
          {paymentTabs.map((tab, index) => (
            <Link href={tab.href} key={index} className="group">
              <TechCard className="cursor-pointer hover:bg-[#2a2d33]/50 transition-colors h-full flex flex-col justify-center">
