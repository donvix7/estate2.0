...
export default function MaintenanceOverviewPage() {
  ...
  useEffect(() => {
    const fetchAllRequests = async () => {
      ...
      const normalizedServices = (serviceData || []).map(s => ({
        ...
        date: s.date || 'Today',
        priority: s.priority || 'Medium', // Default for worker bookings
  ...
  if (isLoading) {
    return <LoadingState message="Scheduling Repairs & Maintenance..." />
  }
    ...
    <div className="flex flex-col lg:flex-row -m-4 lg:-m-8 min-h-[calc(100vh-4rem)]">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto p-6 gap-6 bg-slate-50 dark:bg-transparent">
        ...
        <PageHeader 
          ...
          description="Manage and track all your property repair requests" 
          icon={Wrench}
        >
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className="bg-[#1241a1] hover:brightness-110 text-white px-4 py-2 rounded-md text-sm font-semibold flex items-center gap-2 transition-colors border-none"
            >
            ...
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50">
                {['ID', 'Issue', 'Location', 'Priority', 'Status'].map(h => (
                  <th key={h} className={`px-4 py-3 text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider ${h === 'Priority' ? 'text-center' : ''}`}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <div className="pt-4 flex flex-col gap-2">
              {selected.status !== 'Completed' && (
                <button 
                  ...
                  disabled={isUpdating}
                  className="w-full bg-emerald-600 hover:brightness-110 text-white font-semibold py-3 rounded-md transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 border-none"
                >
                  {isUpdating ? (
                    <>
              ...
              <Link
                href="/dashboard/resident/maintenance/new"
                className="w-full bg-[#1241a1] hover:brightness-110 text-white font-semibold py-3 rounded-md transition-colors text-sm text-center block"
              >
                New Request
              </Link>
