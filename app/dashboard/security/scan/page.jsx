...
export default function QRScanPage() {
 ...
 <div className="flex-1 relative bg-[#0d0f13] rounded-4xl overflow-hidden group shadow-2xl min-h-[400px]">
 <div id={qrCodeRegionId} className="absolute inset-0 w-full h-full" style={{ display: isScannerActive ? 'block' : 'none', background: '#000' }} />
 
 {!isScannerActive && (
 <div className="absolute inset-0 bg-[#0d0f13]/90 flex items-center justify-center">
 ...
 <label className="text-[10px] uppercase font-black text-[#8a8f98] tracking-widest leading-none block">Pass Code / REF</label>
 <div className="relative">
 <input value={passCode} onChange={(e) => setPassCode(e.target.value)} className="w-full bg-[#1a1d23] bg-[#1a1d23]/50 border-none rounded-2xl text-sm font-bold px-5 py-3.5 focus:ring-2 focus:ring-[#1241a1] transition-all outline-none text-white" placeholder="VIS-9928" type="text" />
 <Ticket className="absolute right-4 top-1/2 -translate-y-1/2 text-[#8a8f98] size-4" />
 </div>
 </div>
 <div className="space-y-2">
 <label className="text-[10px] uppercase font-black text-[#8a8f98] tracking-widest leading-none block">PIN</label>
 <div className="relative">
 <input value={pin} onChange={(e) => setPin(e.target.value)} className="w-full bg-[#1a1d23] bg-[#1a1d23]/50 border-none rounded-2xl text-sm font-bold px-5 py-3.5 focus:ring-2 focus:ring-[#1241a1] transition-all outline-none text-white" placeholder="****" type="password" />
 <Lock className="absolute right-4 top-1/2 -translate-y-1/2 text-[#8a8f98] size-4" />
 </div>
 </div>
 <div className="space-y-2">
 <label className="text-[10px] uppercase font-black text-[#8a8f98] tracking-widest leading-none block">License Plate</label>
 <div className="relative">
 <input value={licensePlate} onChange={(e) => setLicensePlate(e.target.value)} className="w-full bg-[#1a1d23] bg-[#1a1d23]/50 border-none rounded-2xl text-sm font-bold px-5 py-3.5 focus:ring-2 focus:ring-[#1241a1] transition-all outline-none text-white" placeholder="KAA 123X" type="text" />
 <Car className="absolute right-4 top-1/2 -translate-y-1/2 text-[#8a8f98] size-4" />
 </div>
 </div>
 ...
 {currentResult ? (
 <Card className={`p-10 text-center space-y-6 relative overflow-hidden animate-in zoom-in duration-300 before:absolute before:top-0 before:left-0 before:right-0 before:h-2 ${currentResult.status === 'denied' ? 'before:bg-red-500' : 'before:bg-emerald-500'}`}>
 ...
 <p className="text-[10px] text-[#8a8f98] uppercase font-black tracking-widest leading-none block mb-1">Gate Status</p>
 <p className="text-sm font-black text-white text-white uppercase tracking-tight">{currentResult.status === 'denied' ? 'Locked' : 'Opening...'}</p>
 </div>
 <div className="text-right">
 <p className="text-[10px] text-[#8a8f98] uppercase font-black tracking-widest leading-none block mb-1">Timestamp</p>
 <p className="text-sm font-black text-white text-white uppercase tracking-tight">{currentResult.timestamp}</p>
 </div>
 </div>
 </Card>
 ) : (
 <Card className="p-10 text-center space-y-6 flex flex-col items-center justify-center min-h-[300px]">
 <div className="size-20 rounded-full bg-[#1a1d23] bg-[#1a1d23] flex items-center justify-center">
 <QrCode className="size-10 text-[#8a8f98] animate-pulse" />
 </div>
 <div>
 <h3 className="text-lg font-black text-[#8a8f98] tracking-tight uppercase">Ready to Scan</h3>
 <p className="text-[#8a8f98] font-medium text-[10px] uppercase tracking-widest mt-2 px-6">System awaiting verification input from scanner or manual terminal</p>
 </div>
 </Card>
 )}
 ...
 <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#8a8f98]">Scan History</h3>
 ...
 <div key={idx} className="flex items-center gap-4 p-4 rounded-2xl bg-[#1a1d23]/30 transition-all cursor-pointer hover:bg-[#1a1d23] hover:bg-[#2a2d33]800/50 group">
 ...
 <p className="text-sm font-black text-white truncate group-hover:text-[#1241a1] transition-colors leading-tight">
 {entry.name || entry.visitorName || 'Unknown Visitor'}
 </p>
 <p className="text-[10px] font-bold text-[#8a8f98] uppercase tracking-widest">
 {entry.type || 'Verification'} • {entry.target || entry.visitorID || 'Main Gate'}
 </p>
 {entry.qrCode && <p className="text-[8px] text-[#8a8f98] truncate opacity-60">ID: {entry.qrCode}</p>}
 </div>
 <div className="text-right">
 ...
 <time className="text-[9px] font-bold text-[#8a8f98] italic block">
 ...
 <div className="px-6 border-t border-[#2a2d33] border-[#2a2d33]800/50">
 <Pagination 
 page={page}
 ...
 <div className="p-4 bg-[#1a1d23]/50">
 <button className="w-full text-center text-[10px] font-black uppercase tracking-widest text-[#8a8f98] hover:text-[#1241a1] transition-colors leading-none">
