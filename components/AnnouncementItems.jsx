...
export default function AnnouncementItem({ icon: Icon, title, desc, time, urgent = false }) {
  return (
    <div className="group flex gap-4 p-4 rounded-md hover:bg-white hover:bg-[#1a1d23]/50 transition-all cursor-pointer text-left">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs text-white0 text-[#8a8f98] line-clamp-2 leading-relaxed">{desc}</p>
        <p className="text-[10px] text-[#8a8f98] mt-1">{time} • Building Management</p>
      </div>
    </div>
  )
}