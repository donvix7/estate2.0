    ...
    <>
      <td className="px-6 py-4">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 rounded-md bg-blue-50 bg-blue-900/50 flex items-center justify-center text-blue-700 text-blue-400 font-semibold text-lg shrink-0 transition-colors group-hover:bg-[#1241a1] group-hover:text-white">
            {worker.name.charAt(0)}
          </div>
          <div>
            <div className="font-semibold text-white text-white">{worker.name}</div>
            <div className="text-xs text-white0 font-mono mt-0.5 font-medium">{worker.employeeId}</div>
          </div>
        </div>
      </td>
      
      <td className="px-6 py-4">
        <div className="font-medium text-white text-white flex items-center gap-2">
          <Briefcase className="w-4 h-4 text-[#8a8f98]" />
          {worker.role}
        </div>
        <div className="text-sm text-white0 mt-1">{worker.title}</div>
      </td>
        ...
        <div className="relative inline-block text-left">
          <button 
            className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all active:scale-90 border-none ${
              activeMenu === (worker._id || worker.id) 
                ...
                : 'bg-[#1a1d23] text-white0 hover:bg-[#2a2d33] '
            }`}
          ...
          {mounted && activeMenu === (worker._id || worker.id) && createPortal(
            <div 
              ...
              className="w-56 rounded-[1.25rem] bg-[#0d0f13] shadow-[0_20px_50px_rgba(0,0,0,0.1)] shadow-[0_20px_50px_rgba(0,0,0,0.4)] overflow-hidden animate-in fade-in zoom-in-95 duration-200"
            >
              <div className="p-2 space-y-1">
                <button 
                  onClick={(e) => { e.stopPropagation(); onRowClick(worker); setActiveMenu(null); }}
                  className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-white text-white hover:bg-[#2a2d33] hover:bg-white/5 rounded-xl transition-colors text-left"
                >
                ...
                <button 
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-white text-white hover:bg-[#2a2d33] hover:bg-white/5 rounded-xl transition-colors text-left"
                >
                  <Edit2 className="w-4 h-4 text-[#8a8f98]" />
                  Edit Info
                </button>
                <button 
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-white text-white hover:bg-[#2a2d33] hover:bg-white/5 rounded-xl transition-colors text-left"
                >
                  <ShieldCheck className="w-4 h-4 text-[#8a8f98]" />
    ...
    <div className="group flex flex-col gap-4">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-md bg-blue-50 bg-blue-900/50 flex items-center justify-center text-blue-700 text-blue-400 font-semibold text-xl shrink-0 transition-colors group-hover:bg-[#1241a1] group-hover:text-white">
            {worker.name.charAt(0)}
          </div>
          <div>
            <h3 className="font-semibold text-white text-white text-base">{worker.name}</h3>
            <p className="text-sm text-white0 font-medium flex items-center gap-1.5 mt-0.5">
      ...
      <div className="grid grid-cols-2 gap-y-3 gap-x-4 bg-[#1a1d23] bg-[#1a1d23]/50 p-3 rounded-md text-sm">
        <div>
          <span className="text-white0 text-xs block mb-1 font-semibold">Department</span>
          <span className="font-semibold text-white text-white">{worker.department}</span>
        </div>
       
        <div className="col-span-2 space-y-1.5 pt-3 mt-1 font-medium">
          <div className="flex items-center gap-2 text-[#8a8f98] text-[#8a8f98]">
