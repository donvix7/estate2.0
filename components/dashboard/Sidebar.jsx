...
import { Building2, Settings, HelpCircle, LogOut, ShieldAlert } from 'lucide-react';
    ...
    <aside className="w-72  bg-slate-50 dark:bg-slate-900/50 flex-col justify-between p-6 shrink-0 overflow-y-auto hidden lg:flex transition-colors">
      <div className="flex flex-col gap-8">
        ...
        <Link href={`/dashboard/${role}/profile`}>
          <div className="size-10 rounded-full bg-slate-300 dark:bg-slate-700 bg-cover bg-center shadow-sm" style={{ backgroundImage: `url(${user?.avatar || 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=100&auto=format&fit=crop'})` }}></div>
          <div className="flex flex-col">
            <h2 className="text-sm font-bold truncate max-w-[120px]">{user?.name || 'User'}</h2>
            <p className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-tight">
              {role === 'admin' ? user?.role || 'Administrator' : `Unit ${user?.unitNumber || '402'} • ${user?.building || 'Phase II'}`}
            </p>
          </div>
        </Link>
        </div>
