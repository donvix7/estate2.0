...
export async function validatePass(codeOrId, pin) {
  try {
    ...
    const updatedPass = await updateRes.json()
    if (!updateRes.ok) throw new Error(updatedPass.message || 'Failed to update pass')
...
export async function handleCreateWorker(workerData) {
  try {
    ...
    const data = await res.json();
    if (!res.ok) return { success: false, message: data.message || 'Failed to create worker' };
    return { success: true, message: 'Service worker created successfully', data };
  } catch (error) {
...
export async function assignWorkerToRequest(id, workerId, residentId) {
  ...
  try {
    ...
    if (!workerRes.ok || workerData.success === false) {
      return { success: false, message: workerData.message || 'Failed to update worker assignment' };
    }
...
export async function handleResidentRequest(formData) {

  try {
    const res = await fetch(`${db_url}/api/resident-request?link=${base_url}/auth/complete-registration`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
