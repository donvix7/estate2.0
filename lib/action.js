...
export async function handleSecurityLogin(email, password, gateId){
    try {
    const res = await fetch(`${db_url}/handleSecurityLogin`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function refreshSecurityToken(){
  try{
    ...
    if (!res.ok || data.success === false) {
      return { success: false, message: data.message || 'Refresh token failed' };
    }
...
export async function deactivateGuestCode(id){
  try{
    ...
    return await handleFetchResponse(res, 'Failed to deactivate guest code');
  } catch (error) {
    return { success: false, message: error.message };
  }   
}
...
export async function preApproveVisitor(id){
  try{
    ...
    return await handleFetchResponse(res, 'Failed to pre-approve guest code');
  } catch (error) {
    return { success: false, message: error.message };
  }   
}
