...
async function fetchWithAuth(url, options = {}) {
  try {
    ...
    const res = await fetch(url, { 
      ...options, 
      headers: { ...headers, ...options.headers, 'x-estate-id': "EST-001" }
    });
...
export async function getAdminSecurityLogs() {
  const data = await fetch(`${db_url}/security-guards/login-status/${attemptId}`);
  if(data.status){
    return data;
  }
  else{
...
export async function getGuestCodesById(id){
  try{
    ...
    return await handleFetchResponse(res, 'Failed to fetch guest codes');
  } catch (error) {
    return { success: false, message: error.message };
  }   
}
